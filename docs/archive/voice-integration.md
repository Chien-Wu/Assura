...
- `npx tsc --noEmit`
- `npm run build`
- Against the local server with an authenticated test cookie: `LEGALMATE_TEST_LEGACY_PROTOCOL=1 node tests/legacy/notes-api.mjs` (see [authentication setup](../authentication.md)). The voice API test requests a connection token but opens no audio connection and sends no participant fields to ElevenLabs.
