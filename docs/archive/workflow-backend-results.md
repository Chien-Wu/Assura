...
程式位置：[case/schema](../../lib/workflow-case.ts)、[後端 API](../../lib/workflow-server.ts)、[API 設定產生器](../../scripts/workflow/workflow-backend-config.mjs)、[隔離實測 runner](../../scripts/experiments/workflow-backend-live.mjs)。完整對接說明：[elevenlabs-workflow-integration.md](elevenlabs-workflow-integration.md)。
