...
GitHub 版本包含可執行的 Web app 與需求文件。文中的原始 `web/` 路徑對應本 repository 根目錄；`reference/forms/` 與 `reference/research/` 仍保留在外層工作區，未隨程式碼上傳。
...
- **正式表單尚未定稿。** 未來由 LegalMate 提供標準 Extra Notes，Provider 勾選需要的表單（見第 10 節）。正式欄位、必填條件、條件式問題、版面及輸出格式仍待確認；`reference/forms/` 是參考材料，不代表已選定正式模板。
