...
| 前端把結果送入 Agent                                              | `app/voice-panel.tsx` 的 `get_form_context`、`update_and_check_form` 工具結果，以及 `sendContextualUpdate()` |
| Agent prompt 另有示範個案專屬提問指示                             | `docs/elevenlabs-system-prompt.txt`；只換掉 `nextQuestions()` 仍會留下舊提問來源                             |
...
- 擴充 `app/provider-roster.tsx` 的個案資料入口，新增文件管理元件與授權 API。
- 改 `lib/audit-server.ts`：保留風險與 evidence capture；移出 `nextQuestions()` 與問號額度的權威角色；新增後端事件 cursor。
- 改 `app/voice-panel.tsx`：新工具、狀態/cursor DTO、獨立檢索生命周期、來源面板；移除 `nextObservationalQuestions` 的背景推送。
