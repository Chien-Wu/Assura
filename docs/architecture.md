...
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Sign-in and test accounts                | `src/lib/auth/`: client, configuration, server session checks and scoped demo accounts.                                                                                                       | `src/components/auth/`                                                          |
| Participants, providers and shifts       | `src/lib/roster/`: profile types, participant validation, organisation scope, scheduling and persistence.                                                                                     | `src/components/manager/roster/`, `src/components/worker/shifts/`               |
| Shift notes and source evidence          | `src/lib/notes/`: form validation, saved notes, safety fields and append-only audit.                                                                                                          | `src/components/worker/notes/`                                                  |
| Recorder                                 | `src/lib/recorder/`: the three client-tool contracts, participant-context read guards, session state and stop/save/review handoff.                                                            | `src/components/worker/recorder/`                                               |
| Silent AI2 and findings                  | `src/lib/assessment/`: result schema, model adapter, prompt, revision-bound assessment, manager decisions and historical types.                                                               | `src/components/worker/notes/risk-review.tsx`, `src/components/manager/review/` |
| Native Workflow test                     | `src/lib/workflow/`: case schema, ordered write queue, sessions and shared six-form persistence.                                                                                              | `src/components/worker/workflow/`                                               |
| Historical context and follow-up records | `src/lib/knowledge/`: context/search, source records and interview evidence.                                                                                                                  | Note reference and audit views                                                  |
| Shared infrastructure                    | `src/lib/shared/server.ts`: one RequestError class, DB access, request identity/origin/body validation and responses. Other shared modules hold class names and server contact configuration. | No screen owns these services.                                                  |

`src/app/` owns URLs and request boundaries. Routes authenticate, parse a request and call the domain service. Database writes repeat ownership and revision checks at execution time. Server modules are imported directly; there are no mixed client/server barrel files. Pure validators keep relative TypeScript imports so the browser build and Node test runner can both use them.
...
- Recorder `get_participant_context` reads the authenticated note's knowledge context after queued saves. Session, note revision, worker-turn and interruption checks discard stale responses. Retrieval provenance is persisted; ordinary recorder follow-up remains in the transcript and does not populate the retained legacy interview-question ledger.
