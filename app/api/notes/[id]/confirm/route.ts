...
export async function POST(request:Request,context:{params:Promise<{id:string}>}) {
  try {
    ...
    let sessionGuard="";const sessionBindings:(string|number)[]=[];
    if(body.voiceSessionId!==undefined){
      const session=await getVoiceSession(body.voiceSessionId,user.userId,id);
      try{evidence={...voiceEvidence(JSON.parse(session.state_json),body.confirmationId,row.revision),sessionId:session.id,conversationId:session.conversation_id};}
      catch(error){throw new RequestError(error instanceof Error?error.message:"Voice confirmation is missing.",409);}
      sessionGuard=" AND EXISTS (SELECT 1 FROM voice_sessions WHERE id=? AND owner_id=? AND note_id=shift_notes.id AND revision=? AND expires_at>?)";
      sessionBindings.push(session.id,user.userId,session.revision,new Date().toISOString());
    }
    const now=new Date().toISOString();
    const result=await database().prepare("UPDATE shift_notes SET status='complete',confirmed_at=?,updated_at=?,confirmation_evidence=? WHERE id=? AND owner_id=? AND revision=? AND review_version=revision AND confirmation_id=? AND status='draft'"+sessionGuard).bind(now,now,JSON.stringify(evidence),id,user.userId,row.revision,body.confirmationId,...sessionBindings).run();
