...
const read = () =>
  markReadback(
    add(
      prepared(),
      "agent",
      "To save this note, say I confirm this shift note, or tell me what to change.",
    ),
    "current",
    1,
  );
test("only a fresh full confirmation after the current review completes a note", () => {
  ...
  assert.throws(() =>
    voiceEvidence({ ...confirmed, closed: true }, "current", 2),
  );
  assert.equal(
...
test("typed confirmation retains the same revision checks and is labelled as text", () => {
  ...
  assert.equal(evidence.method, "text");
  assert.equal(evidence.source, "browser_text_input");
  assert.throws(() => voiceEvidence(confirmed, "current", 3));
  assert.throws(() =>
    voiceEvidence(
      add(confirmed, "user", "Actually change the time"),
      "current",
      2,
    ),
  );
...
test("yes, silence, and an old confirmation cannot complete the draft", () => {
  ...
  state = markReadback(
    add(state, "agent", "Say I confirm this shift note."),
    "current",
    2,
  );
...
test("an early correction or early confirmation invalidates pending readback", () => {
  for (const text of [
    ...
    const state = add(
      add(prepared(), "user", text),
      "agent",
      "Say I confirm this shift note.",
    );
