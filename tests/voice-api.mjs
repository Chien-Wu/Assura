...
async function api(path,body,expected=200,method=body===undefined?"GET":"POST"){
  ...
  assert.equal(response.status,expected,`${path}: ${data.error||response.status}`);checks++;
  return data;
}
...
const signedInCookie=cookie;cookie="";
await api(path,{action:"close"},401);cookie=signedInCookie;
await api(`/api/voice/sessions/${randomUUID()}`,{action:"close"},404);
let sequence=0;
const event=(kind,text)=>api(path,{action:"event",event:{sequence:++sequence,kind,text}});
const prepare=async()=>{
  const review=await api(`/api/notes/${id}/review`,{revision:1});
  await api(path,{action:"prepare",confirmationId:review.confirmationId,revision:1});return review;
};
const confirm=(review,expected)=>api(`/api/notes/${id}/confirm`,{revision:1,confirmationId:review.confirmationId,confirmed:true,voiceSessionId:session.sessionId},expected);
...
await api(path,{action:"readback",confirmationId:review.confirmationId,sequence},409);
await confirm(review,409);
review=await prepare();
await event("agent","Say I confirm this shift note.");
await api(path,{action:"readback",confirmationId:review.confirmationId,sequence});
await event("user","Yes");await confirm(review,409);
review=await prepare();
await event("agent","Say I confirm this shift note.");
await api(path,{action:"readback",confirmationId:review.confirmationId,sequence});
await event("interrupt","Worker interrupted");await confirm(review,409);
review=await prepare();
await event("agent","Say I confirm this shift note.");
await api(path,{action:"readback",confirmationId:review.confirmationId,sequence});
await event("user","I confirm this shift note.");
const completed=await confirm(review,200);assert.equal(completed.note.status,"complete");
const retry=await confirm(review,200);assert.equal(retry.note.confirmedAt,completed.note.confirmedAt);
await api(path,{action:"close"});await api(path,{action:"event",event:{sequence:++sequence,kind:"user",text:"After close"}},409);
await api("/api/voice/sessions",{noteId:id},409);
console.log(JSON.stringify({passed:true,httpChecks:checks,testNoteId:id,connectionTokenReceived:true,audioConnectionStarted:false}));
